...
std::mutex mutex;
std::map<uint32_t, int> sockets_ids;
...
void handleClient(Hotel &hotel, int client_fd) {
    ...
    while (true) {
        if (read(client_fd, (void *) &pkg_id, sizeof(pkg_id)) <= 0 || socket_fd == -1) {
            break;
        }
        ...
        if (pkg_id == 1) {
            ...
            sockets_ids[client_id] = client_fd;
            ...
            mutex.lock();
            handleTermination(0);
            std::cout << "[SERVER] Terminating server by manager request!" << std::endl;
            mutex.unlock();
        } else if (pkg_id == 4) {
            mutex.lock();
            uint32_t received_client_id = readUInt32FromFd(client_fd);

            if (sockets_ids.find(received_client_id) != sockets_ids.end()) {
                std::cout << "[SERVER] Disconnecting client with id = " << received_client_id << std::endl;
                shutdown(sockets_ids[received_client_id], SHUT_RDWR);
                sockets_ids.erase(received_client_id);
            }
